...
class jad_matrix : public detail::matrix_base<IndexType,ValueType,MemorySpace,cusp::jad_format>
{
  typedef cusp::detail::matrix_base<IndexType,ValueType,MemorySpace,cusp::jad_format> Parent;
  public:
    /*! rebind matrix to a different MemorySpace
     */
    template<typename MemorySpace2>
    struct rebind { typedef cusp::jad_matrix<IndexType, ValueType, MemorySpace2> type; };

    /*! type of column indices array
     */
    typedef typename cusp::array1d<IndexType, MemorySpace> indices_array_type;
     ...
     */
    typedef typename cusp::jad_matrix<IndexType, ValueType, MemorySpace> container;
...
//                                           IndexType, ValueType, MemorySpace> const_view;
    ...
    indices_array_type permutations;

    indices_array_type diagonal_offsets;

    /*! Storage for the column indices of the ELL data structure.
     */
    indices_array_type column_indices;
     ...
     */
    jad_matrix(size_t num_rows, size_t num_cols, size_t num_entries,
               size_t num_jagged_diagonals)
      : Parent(num_rows, num_cols, num_entries)
    {

        permutations.resize(num_rows);
        diagonal_offsets.resize(num_jagged_diagonals+1);
      column_indices.resize(num_entries);
      values.resize        (num_entries);
      this->num_jagged_diagonals = num_jagged_diagonals;
    }
    ...
    jad_matrix(const MatrixType& matrix);
     ...
     */
    template <typename MatrixType>
    jad_matrix& operator=(const MatrixType& matrix);
}; // class ell_matrix
/*! \}
 */
...
#include <cusp/detail/jad_matrix.inl>
